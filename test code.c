...
task checkControls() {
	while(true) {
		...
		if(vexRT[Btn8D] == 1) { //if up button is pressed raise the arm until sensor is pressed
			...
			wait1Msec(500);//waitUntil(nMotorEncoder[rightTwist] >= 1 * ONE_ROTATION);
			setTwistMotors(0);
...
task twistTest() {
	while(true) {
		if(vexRT[Btn6U] == 1) {//right
			motor[rightTwist] = 10;//up
		} else if(vexRT[Btn6D] == 1) {
			motor[rightTwist] = -10;//down
		}
		if(vexRT[Btn5U] == 1) {//left
			motor[leftTwist] = 10;//up
		} else if(vexRT[Btn5D] == 1) {
			motor[leftTwist] = -10;//down
		}
	}
}
